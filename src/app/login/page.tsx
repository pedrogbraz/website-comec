...
export default function Login() {
  ...
  const [subtitle, setSubtitle] = useState<string>(
    "Insira seus dados pessoais para usufruir de todo o site."
  );
  ...
  function mudarLogin() {
    if (title === "Bem vindo de volta!") {
      ...
      setSubtitle(
        "Registre-se com seus dados pessoais para usufruir de todo o site."
      );
  ...
  return (
    <main className="w-full h-[100vh] flex">
      <div className="flex -z-50 absolute w-full h-full">
        <Image
          src={`${showLogin ? "/ondas1.png" : "/ondas1.png"}`}
          alt="Teste"
          layout="fill"
          objectFit="cover"
          className={``}
      ...
      <div
        ...
        } w-[100%] lg:w-[60%] px-20 flex-col gap-4 md:gap-2 justify-center items-center`}
      >
        <h1 className="text-center text-2xl font-semibold text-[--login]">
          Entrar
        </h1>
        <Input
          className="border"
        ...
        <div className="flex justify-between items-center w-[100%] px-2">
          <Button
            ...
            className="text-[--color-text] px-0 text-sm font-normal"
          >
            Não tem conta?
          </Button>
          <Link href="/forgot-password" className="text-[--color-text] text-sm">
            Esqueceu sua senha?
          </Link>
        </div>
        <Button className="md:text-lg px-12 py-6 w-[100%] bg-[--login] hover:bg-[--login]/90">
          Entrar
        </Button>
      </div>
      <div
        className={`${
          showLogin ? "flex" : "hidden"
        } hidden md:flex w-[50%] h-[100vh] bg-[--login] ${
          showLogin ? "rounded-l-[120px]" : "rounded-r-[120px]"
        } flex-col items-center gap-2 justify-center`}
      >
        <div className="absolute flex flex-col items-center">
          <h1 className="text-[--loginButton] font-bold text-3xl md:text-xl">
            {title}
          </h1>
          <p className="text-[--loginButton] text-center">{subtitle}</p>
          <Button
            className="mt-4 px-8 py-6 md:px-6 md:py-4 text-[--login] bg-[--loginButton] hover:bg-[--loginButton]/90"
            onClick={toggleSections}
          >
            {textButton}
          </Button>
      ...
      <div
        className={`${
          showLogin ? "hidden" : "flex"
        } w-[100%] lg:w-[50%] flex-col gap-2 justify-center px-10 items-center`}
      >
        <h1 className="text-center text-2xl font-semibold text-[--blue-theme]">
          Cadastro
        </h1>
        <Input
          className="border"
        ...
        <Input
          className="border"
          type="text"
          placeholder="Digite seu nome de crachá"
        />
        <Input
          className="border"
          type="text"
        ...
        <div className="w-[100vw] md:w-[100%] px-10 md:px-0 flex gap-4 md:gap-2">
          ...
          <Select>
            ...
            <SelectContent>
              <SelectItem value="ACADÊMICO SÓCIO DA SAMPI">
                ACADÊMICO SÓCIO DA SAMPI
              </SelectItem>
              <SelectItem value="ACADÊMICO NÃO SÓCIO DA SAMPI">
                ACADÊMICO NÃO SÓCIO DA SAMPI
              </SelectItem>
              <SelectItem value="MÉDICO SÓCIO DA SAMPI">
                MÉDICO SÓCIO DA SAMPI
              </SelectItem>
              <SelectItem value="MÉDICO NÃO SÓCIO DA SAMPI">
                MÉDICO NÃO SÓCIO DA SAMPI
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        />
        <Button className="mt-4 md:text-lg px-8 py-6 w-[100%] bg-[--login] hover:bg-[--login]/90">
          Cadastrar
        </Button>
      </div>
    </main>
